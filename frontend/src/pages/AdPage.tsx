        ...
        <Card className="w-full min-h-screen p-6 rounded-xl shadow-lg">
            <div className="flex flex-col gap-6 w-[65%] mx-auto">
                ...
                <Image
                    ...
                    preview={false}
                    placeholder={<Image preview={false} src={defaultImage} width={545} height={450} />}
                    className="rounded-lg object-cover object-center"
                />
                </div>

                <div className="flex flex-col justify-between">
                    <Title level={1} className="font-bold text-gray-900 line-clamp-3" title={ad.name}>
                        {ad.name}
                    </Title>
                    ...
                    <Button text="Редактировать" fn={handleEdit} styles="w-52 h-14 bg-[#f1f1f1] hover:bg-[#e5e4e4]"/>
                    </div>
                </div>
                </div>
                <Text className="mt-3 text-gray-600 text-sm leading-relaxed">{ad.description}</Text>
            </div>
        </Card>
    );
};
